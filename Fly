-- I did not make this one, also IY's fly is better than this lmao. Decompiled from KAH
	...
	while isFlying do
		...
		if movementVector.l + movementVector.r == 0 then
			if movementVector.f + movementVector.b ~= 0 then
				mathObj = mathObj + 0.5 + mathObj / 50
				if 50 < mathObj then
					mathObj = 50
				end
		...
		bodyGyro.cframe = game.Workspace.CurrentCamera.CoordinateFrame * CFrame.Angles(-math.rad((movementVector.f + movementVector.b) * 50 * mathObj / 50), 0, 0)
			if isFlying then
		else
			break
		end		
